...
define(["jquery", "lodash", "form2js", "org/forgerock/openidm/ui/admin/authentication/AuthenticationAbstractView", "org/forgerock/openidm/ui/admin/delegates/ExternalAccessDelegate", "libs/codemirror/lib/codemirror", "libs/codemirror/mode/xml/xml"], function ($, _, Form2js, AuthenticationAbstractView, ExternalAccessDelegate, codemirror) {
    ...
    var OpenIDConnectView = AuthenticationAbstractView.extend({
        ...
        knownProperties: AuthenticationAbstractView.prototype.knownProperties.concat(["openIdConnectHeader", "resolvers"]),

        handleWellKnownChange: function handleWellKnownChange(e) {
            var _this = this;

            ExternalAccessDelegate.externalRestRequest($(e.target).val()).then(function (config) {
                _.keys(config).forEach(function (k) {
                    ...
                    _this.$el.find("[name='properties.resolvers[]." + k + "']").val(config[k]);
                });
        ...
        render: function render(args) {
            ...
            ExternalAccessDelegate.externalRestRequest(this.$el.find("input[name='properties.resolvers[].well-known']").val()).then(function (config) {
        ...
        renderTemplate: function renderTemplate() {
            ...
            this.parentRender(function () {
                _this3.postRenderComponents({
                    "customProperties": _this3.data.customProperties,
                    "name": _this3.data.config.name,
                    "augmentSecurityContext": _this3.data.config.properties.augmentSecurityContext || {},
                    "userOrGroup": _this3.data.userOrGroupDefault
                });

                _this3.model.iconCode = codemirror.fromTextArea(_this3.$el.find(".button-html")[0], {
                    lineNumbers: true,
