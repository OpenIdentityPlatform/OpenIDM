...
if not "%JAVA_OPTS_PARAM%" == "" goto optsparamOK
set JAVA_OPTS_PARAM=-Dopenidm.system.server.root="%LAUNCHER_SERVER_HOME%";
:optsparamOK
...
:srvInstall
rem Install OpenIDM as Windows service
shift
set SERVICE_NAME=OpenIDM
if not ""%1"" == """" (
    set SERVICE_NAME=%1
)
shift
...
"%LAUNCHER_SERVER_HOME%\bin\%ARCH%\prunsrv.exe" //IS//%SERVICE_NAME% --Install="%LAUNCHER_SERVER_HOME%\bin\%ARCH%\prunsrv.exe" --Description="OpenIDM 2.1.x Service" --Startup=auto --Jvm=%JAVA_DLL% --Classpath=%CP% --JvmOptions=%JAVA_OPTS_SERVICE%%JAVA_OPTS_PARAM% --StartPath="%LAUNCHER_SERVER_HOME%" --StartMode=jvm --StartClass=%MAIN_CLASS% --StartMethod=start --StartParams="%LAUNCHER_START_PARAMS%" --StopMode=jvm --StopClass=%MAIN_CLASS% --StopMethod=stop --LogPath="%LAUNCHER_SERVER_HOME%\logs" --LogPrefix=launcher --StdOutput=auto --StdError=auto --LogLevel=Warn
echo OpenIDM Service successfully installed as "%SERVICE_NAME%" service
goto :EOF
