...
function atLeastLengthValidator(toValidate, minLength, fieldName) {
	if (toValidate.length < minLength) {
		throw fieldName + " should have at least " + minLength + " characters";
    }
}
...
function alLeastOneNumberValidator(toValidate, fieldName) {
    ...
    if (!reg.test(toValidate)) {
    	throw fieldName + " should have at least one number";
    }
}
...
function numbersAndSpecialCharsValidator(toValidate, fieldName) {
	...
	var reg = /^\+?([0-9\- \(\)])*$/;
    if (!reg.test(toValidate)) {
    	throw "Only numbers and special characters allowed in " + fieldName;
    }
}
...
function isUserValid() {
    ...
    requiredOnlyAlfabeticalValidator(user.firstname, "Firstname");
    requiredOnlyAlfabeticalValidator(user.lastname, "Lastname");
