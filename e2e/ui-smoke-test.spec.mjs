...
async function clickDropdownItem(page, dropdownLabel, itemHref) {
    ...
    // Wait for the parent <li> to gain the "open" class (Bootstrap 3 dropdown behavior)
    const parentLi = toggle.locator("..");
    await expect(parentLi).toHaveClass(/open/, { timeout: 10000 });

    // Use a partial href match (ends-with) to be resilient to base-path differences
...
test.describe("Admin UI - Navigation Menu", () => {
    ...
    test("Manage dropdown opens and navigates to Users list", async ({ page }) => {
        ...
        // Use a partial href match for managed user list
        const usersLink = page.locator('.dropdown-menu a[href$="resource/managed/user/list/"]').first();
