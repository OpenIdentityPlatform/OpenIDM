...
The `OPENID_CONNECT` module authenticates users via an OpenID Connect 1.0 provider. It reads a JWT access token from an HTTP header, validates it against the configured OIDC provider, and maps the identity to an OpenIDM managed object.
