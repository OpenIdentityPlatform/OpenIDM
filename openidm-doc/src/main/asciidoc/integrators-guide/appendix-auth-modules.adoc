...
a|Query on Resource
a|managed/google
a|Managed object endpoint to query after token validation.
a|authentication.json
...
a|Well-Known URL
a|(blank)
a|OIDC Discovery URL (for example, `https://accounts.google.com/.well-known/openid-configuration`). When provided, the module auto-populates `authorization_endpoint`, `token_endpoint`, and `userinfo_endpoint`.
a|authentication.json
...
a|Authentication ID
a|sub
a|Claim name in the identity provider profile or ID token used as the user identifier.
a|authentication.json
