...
[#social-providers-module-prop-nested]
.SOCIAL_PROVIDERS Module Properties (under `properties`)
[cols="18%,27%,27%,28%"]
|===
|Admin UI Label |Default |Description |Configuration File
...
a|User Roles
a|authzRoles
a|Attribute used for authorization roles, inherited by all generated modules.
a|authentication.json

a|Default User Roles
a|openidm-authorized
a|List of roles inherited by all generated modules.
a|authentication.json
|===
