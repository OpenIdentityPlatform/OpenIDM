...
public class OpenICFProvisionerService implements SingletonResourceProvider, ConnectorEventHandler {
    ...
    private AtomicBoolean serviceAvailable = new AtomicBoolean(Boolean.FALSE);
   ...
   private String systemName = null;
    ...
    protected void activate(ComponentContext context) {
        try {
            ...
            if (configuration.isDefined("name")) {
                systemName = configuration.get("name").required().asString();
            } else {
                systemName = (String) context.getProperties().get("config.factory-pid");
            }

            allowModification = !configuration.get("readOnly").defaultTo(false).asBoolean();
            ...
            ConnectorInfo connectorInfo =
                    connectorInfoProvider.findConnectorInfo(connectorReference);
            if (null != connectorInfo) {
                logger.info("OpenICF ConnectorInfo of {} was found.", connectorReference);
                init(connectorInfo, configuration);
            } else if (connectorReference.getConnectorLocation().equals(
                    ConnectorReference.ConnectorLocation.LOCAL)) {
                logger.error("OpenICF ConnectorInfo can not be loaded for {} from #LOCAL",
                        connectorReference);
                throw new ComponentException("OpenICF ConnectorInfo can not be retrieved for "
                        + connectorReference);
            } else {
                /*
                 * This should never happen because the configuration has to be
                 * encrypted and the encryption requires the ConnectorInfo so
                 * it's a safe assumption. If this block is executed then there
                 * was some change in the initialisation mechanism.
                 */
                logger.info("OpenICF ConnectorInfo for {} is not available yet.",
                        connectorReference);
            }
            if (!connectorReference.getConnectorLocation().equals(
                    ConnectorReference.ConnectorLocation.LOCAL)) {
            ...
            // TODO
            // router.addRoute(RoutingMode.EQUALS, "/system/OpenDJ", this);
            // router.addRoute(RoutingMode.EQUALS, "/system/OpenDJ/{type}",
            // this);
            // router.addRoute(RoutingMode.EQUALS, "/system/OpenDJ/{type}/{id}",
            // this);

            logger.info("OpenICF Provisioner Service component {} is activated{}", systemName,
                    (serviceAvailable.get() ? "." : " although the service is not available yet."));
        } catch (Exception e) {
    ...
    private RouteBuilder init(ConnectorInfo connectorInfo, JsonValue configuration) {
        ...
        try {
            ...
            for (Map.Entry<String, ObjectClassInfoHelper> entry : ConnectorUtil.getObjectTypes(
                ...
                builder.withModeEquals().withTemplate(
                        "/system/" + systemName + "/" + entry.getKey())
                        .withCollectionResourceProvider(
                                new ObjectClassResourceProvider(entry.getValue(), objectOperations
                                        .get(entry.getKey()))).buildNext();
            }
            builder.withModeEquals().withTemplate("/system/" + systemName).withSingletonResourceProvider(this).seal();
            //TODO Fix this Map
            //ValidateApiOp
            //TestApiOp
            //ScriptOnConnectorApiOp
        ...
        ConnectorFacade facade = getConnectorFacade(connectorInfo, configuration);
        if (null != facade && facade.getSupportedOperations().contains(TestApiOp.class)) {
            try {
                //TODO make configurable to not throw exception here
                facade.test();
    ...
    ConnectorFacade getConnectorFacade(ConnectorInfo connectorInfo, JsonValue configuration) {
        if (null == connectorFacade) {
            ...
            connectorFacade = connectorFacadeFactory.newInstance(config);
        }
    ...
    protected void deactivate(ComponentContext context) {
        ...
        //TODO FixMe
        logger.info("OpenICF Provisioner Service component {} is deactivated.", "//TODO FIXME");
    }
    ...
    /**
     * Gets a brief stats report about the current status of this service newBuilder.
     * </p/>
     * {@code {
     * "name" : "LDAP",
     * "component.id" : "1",
...
//    }
     ...
     */
    org.forgerock.openidm.smartevent.Name getQueryEventName(String objectClass, JsonValue params, Map<String, Object> query, String queryId) {
    ...
    /**
     * This newBuilder and this method can not be scheduled. The call MUST go through the
     ...
     */
    public JsonValue liveSynchronize(String objectType, JsonValue previousStage, final ResultHandler<Resource> synchronizationListener) {
        ...
        if (logger.isTraceEnabled()) {
            if (null != request) {
                try {
                    ...
                    MAPPER.writeValue(writer, request.getResourceName());
    ...
    public void actionInstance(ServerContext context, ActionRequest request,
        ...
        try {
            if (ConnectorAction.script.name().equalsIgnoreCase(request.getActionId())) {
                // TODO NPE check
                SystemAction action =
                        systemActions.get(request.getAdditionalActionParameters().get(
                ...
                if (null != scriptContextBuilderList) {
                    OperationHelper helper = null;// operationHelperBuilder
    ...
    private class ObjectClassResourceProvider implements CollectionResourceProvider {
        ...
        private ObjectClassResourceProvider(ObjectClassInfoHelper objectClassInfoHelper, Map<Class<? extends APIOperation>, OperationOptionInfoHelper> operations) {
         ...
         *                            {@link org.forgerock.openidm.provisioner.openicf.commons.OperationOptionInfoHelper.OnDenyAction#THROW_EXCEPTION}
         */
        private <V> ConnectorFacade getConnectorFacade0(final ResultHandler<V> handler, Class<? extends APIOperation> operation)
            ...
            ResourceException e = null;
             ...
             if (null == facade.getOperation(operation)) {
                e = new NotSupportedException("Operation " + operation.getCanonicalName()
                        + " is not supported by the Connector");
            } else if (null != operationOptionInfoHelper
        ...
        public void queryCollection(final ServerContext context,final QueryRequest request,final QueryResultHandler handler) {
            EventEntry measure = null;//Publisher.start(getQueryEventName(id, params, query.asMap(), queryId.asString()), null, id);
            try {
                ...
                if (null != facade) {
                    ...
                    if (request.getQueryId() != null) {
                        if (!ServerConstants.QUERY_ALL_IDS.equals(request.getQueryId())) {
                            ...
                            handler.handleError(new BadRequestException("Unsupported _queryId: " + request.getQueryId()));
                            return;
                        }
                    ...
                    facade.search(objectClassInfoHelper.getObjectClass(), filter, new ResultsHandler() {
                        @Override
        ...
        public void readInstance(ServerContext context, String resourceId, ReadRequest request, ResultHandler<Resource> handler) {
            try {
                ...
                if (null != facade) {
                    ...
                    ConnectorObject connectorObject =  facade.getObject(objectClassInfoHelper.getObjectClass(), uid, operationOptions);
...
/*        public JsonValue update(Id id, String rev, JsonValue object, JsonValue params) throws Exception {
    ...
    private static final QueryFilterVisitor<Filter,ObjectClassInfoHelper> RESOURCE_FILTER = new QueryFilterVisitor<Filter,ObjectClassInfoHelper>() {
