...
public class IWAPassthroughModule extends IWAModule {
    ...
    protected void doInitialize(MessagePolicy requestPolicy, MessagePolicy responsePolicy, CallbackHandler handler,
        ...
        super.doInitialize(requestPolicy, responsePolicy, handler, options);
...
//            LOGGER.debug("IWAADPassthroughModule: Have OpenIDM username, falling back to AD Passthrough");
            return adPassthroughModule.doValidateRequest(messageInfo, clientSubject, serviceSubject, authData);
        }

        try {
            AuthStatus authStatus = super.doValidateRequest(messageInfo, clientSubject, serviceSubject, authData);
