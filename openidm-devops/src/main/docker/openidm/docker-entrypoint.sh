...
if [ "$1" = 'openidm' ]; then
    ...
    BOOT_OPTS="-Dopenidm.keystore.password=$KST_PASS -Dopenidm.truststore.password=$TST_PASS -Dopenidm.node.id=$NODE_ID -Dopenidm.db.host=$DB_HOST -Dopenidm.db.port=$DB_PORT -Dopenidm.db.user=$DB_USER -Dopenidm.db.password=$DB_PASSWORD"
