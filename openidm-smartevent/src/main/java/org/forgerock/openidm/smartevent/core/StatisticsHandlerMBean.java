 ...
 * 
 * Provide JMX / MBean access for monitoring and management
 * of SmartEvents and in turn the instrumented application
 * 
 * @author aegloff
 */
public interface StatisticsHandlerMBean {
    ...
    /**
     * @return the recent history of events, 
     * mapping from start time to the event detail
     */
    Map getRecent();
     ...
     * @param eventName the event name in Stringified notation
     */
    void resetStatistics(String eventName);
    
    /**
     * Set the enabled status for a given event name
     * Disabling an event means no consumer will get notified
     * @param eventName the event name in Stringified notation
     ...
     * @param eventName the event name in Stringified notation
     * @param enabled true to keep result details in the recent event history
     * How long it is kept depends on the history size and the frequency of new events
     * Caution should be exercised on events with large result objects as it 
     * will affect system memory consumption
     */
    void setResultHistoryEnabled(String eventName, boolean enabled);
    
    /**
     * @return A map from Stringified event name to 
     * its result history enabled status
     */
