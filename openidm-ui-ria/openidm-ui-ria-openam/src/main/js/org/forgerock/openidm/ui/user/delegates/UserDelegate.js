...
define("UserDelegate", [
    ...
    obj.login = function(uid, password, successCallback, errorCallback, errorsHandlers) {
        ...
        obj.serviceCall({
            serviceUrl: configuration.globalData.openamBaseURL + "/identity/authenticate",
            ...
            success: _.bind(function (data) {
                ...
                if(!data.length || !data.match(/^token\.id=/)) {
                    ...
                    cookieHelper.setCookie("iPlanetDirectoryPro", token, "", "/", document.domain.split(".").splice(1).join(".")); // the name of this cookie should probably be in the config as well 
    ...
    obj.logout = function() {
        ...
        obj.serviceCall({
            ...
            type: "GET",
            success: function () { 
                cookieHelper.deleteCookie("iPlanetDirectoryPro", "/", document.domain.split(".").splice(1).join("."));
            }
    });
